...
| spawnAnimation   | `level`: Int, tileX`: Int, `tileY`: Int, `offsetX`: Numeric, `offsetY`: Numeric, `file`: String, `center`: Bool | Int | Spawns a non-collidable animation entity at the given position. Origin is either center or top left. Returns the id of the entity. Does not play the animation |
